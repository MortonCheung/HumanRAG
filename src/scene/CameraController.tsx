...
type View = { position: THREE.Vector3; target: THREE.Vector3; intentId: string };
...
export function CameraController({ intent, model, experiencePhase, motionAllowed, onEntryComplete, skipVersion }: Omit<CameraLifecycle, 'onReady'> & {
  ...
  useEffect(() => {
    ...
    const remember = () => {
      ...
      lastUniverseView = { position: instance.getPosition(new THREE.Vector3(), false), target: instance.getTarget(new THREE.Vector3(), false), intentId: current.current.intent.id };
    };
    const takeControl = () => {
    ...
    instance.addEventListener('controlstart', takeControl);
    instance.addEventListener('rest', remember);
    return () => { remember(); instance.removeEventListener('controlstart', takeControl); instance.removeEventListener('rest', remember); };
  }, []);
  ...
  useLayoutEffect(() => {
    ...
    if (wasEntering && experiencePhase === 'universe') {
      ...
      lastUniverseView = { position: instance.getPosition(new THREE.Vector3(), false), target: instance.getTarget(new THREE.Vector3(), false), intentId: intent.id };
      return;
    }
    ...
    const apply = (position: THREE.Vector3, target: THREE.Vector3, smooth = false) => {
      void instance.setLookAt(position.x, position.y, position.z, target.x, target.y, target.z, smooth && motionAllowed);
      invalidate();
    };
    ...
    if (intent.mode !== 'node') { apply(overview, center, !initial); return; }
  ...
  }, [experiencePhase, intent.id, size.width, size.height, motionAllowed, camera, invalidate]);
  ...
  return <CameraControls ref={controls} enabled={experiencePhase !== 'landing'} makeDefault minDistance={4} maxDistance={240} minPolarAngle={0.05} maxPolarAngle={Math.PI - 0.05} smoothTime={motionAllowed ? 0.48 : 0} draggingSmoothTime={0.08} dollySpeed={0.8} truckSpeed={1} azimuthRotateSpeed={0.68} polarRotateSpeed={0.62} mouseButtons={{ left: CameraControlsImpl.ACTION.ROTATE, middle: CameraControlsImpl.ACTION.DOLLY, right: CameraControlsImpl.ACTION.TRUCK, wheel: CameraControlsImpl.ACTION.DOLLY }} touches={{ one: CameraControlsImpl.ACTION.TOUCH_ROTATE, two: CameraControlsImpl.ACTION.TOUCH_DOLLY_TRUCK, three: CameraControlsImpl.ACTION.TOUCH_TRUCK }} />;
}
